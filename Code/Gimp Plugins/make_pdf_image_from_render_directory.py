...
class Page:
    ...
    def __init__(self, pageId, outdir):
...
def make_pdf_image_from_render_directory(image, drawable, sourceDirectory, outDirectory):
    ...
    for renderDirectory in os.listdir(sourceDirectory):
        dirname = sourceDirectory + '\\' + renderDirectory + '\Merged'
        if not os.path.exists(dirname):
            os.makedirs(dirname)
        filenames = os.listdir(sourceDirectory + '\\' + renderDirectory)
...
register(
        ...
        "RGB*, GRAY*",
        [
                (PF_STRING, "sourceDirectory", "Source Directory", rootPath),
